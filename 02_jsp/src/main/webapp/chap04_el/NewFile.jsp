...
<%--
  ...
  3. 서블릿 컨테이너의 각 영역(page, request, session, application)에 저장된 값을 쉽게 접근할 수 있도록 설계되었습니다.
  ...
  7. EL 내장 객체를 활용할 수 있습니다.
     ...
     --------------------------------------------------------------------------------------------------------------------------------------------
     1) pageScope          현재 JSP 페이지에서 선언한 객체에 접근할 때 사용                                 ${pageScope.속성}
     2) requestScope       하나의 요청(request) 동안 공유되는 객체에 접근할 때 사용                         ${requestScope.속성}
     3) sessionScope       로그인 등 사용자별 세션 동안 공유되는 객체에 접근                                ${sessionScope.속성}
     4) applicationScope   웹 애플리케이션 전체에서 공유되는 전역 객체에 접근                               ${applicationScope.속성}
     ...
     8) pageContext        PageContext 인터페이스를 구현한 클래스의 객체(다른 영역 접근 가능)               ${pageContext}
     9) request            HttpServletRequest 인터페이스를 구현한 클래스의 객체(요청 관련 메서드 사용 가능) ${pageContext.request.메소드}
     10) session           HttpSession 인터페이스를 구현한 클래스의 객체(세션 관련 메서드 사용 가능)        ${pageContext.session.메소드}
     11) servletContext    ServletContext 인터페이스를 구현한 클래스의 객체(서버 전체 범위 설정 등 접근)    ${pageContext.servletContext.메소드}
 --%>
